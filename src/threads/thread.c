...
void update_load_avg_and_recent_cpu(void) {
  ...
  Ele=list_begin(&all_list);
  while(Ele != list_end(&all_list)){
    Thr = list_entry(Ele, struct thread, allelem);
    if (Thr != idle_thread) 
      Thr->Recent_Cpu = Float_ADD_Int(Float_MUL_Float(Float_DIV_Float(Int_MUL_Float(2, load_avg), Float_ADD_Int(Int_MUL_Float(2, load_avg), 1)), Thr->Recent_Cpu), Thr->Nice);
...
void update_priority() {
  ...
  while(Ele != list_end(&all_list)){
    Thr = list_entry(Ele, struct thread, allelem);
    Thr->priority = Float_SUB_Float(Float_SUB_Float(Float_ADD_Int(0, PRI_MAX), Float_DIV_Int(Thr->Recent_Cpu, 4)), Int_MUL_Float(2, Float_ADD_Int(0, Thr->Nice))) / FRACTION;
...
thread_get_load_avg (void) 
{
  ...
  int res=Int_MUL_Float(100, load_avg) / FRACTION;
   return res;
}
...
thread_get_recent_cpu (void) 
{
  ...
  int res=Int_MUL_Float(100, thread_current()->Recent_Cpu) / FRACTION;
  return res;
}

